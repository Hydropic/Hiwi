...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %Nicht jeder Splinepunkt wird getestet, deshalb hilfsvariablen


    %%% Winkel Bedingung
    Sym_zu_TCP = simulation_data(:,1:3);%Winkel zwischen Sym  und TCP

    kellen_orientierung_beschleunigung_neu = [rx ry rz direction(1) direction(2) direction(3) acceleration]; % Rx Ry Rz nx ny nz A
    %%%kelle_Welt_zu_TCP = kellen_orientierung_beschleunigung_neu(:,1:3);%Weltkoord!!
    kelle_Welt_zu_Sym = [rx, ry, 0]; %Trans Welt Sym?????
    kelle_richtung_Welt_neu = kellen_orientierung_beschleunigung_neu(:,4:6);%Weltkoord!!

    [k,dist] = dsearchn(table2array(Sym_zu_TCP),kelle_Welt_zu_Sym);
  
    simu_CFD_id_k = simulation_data(k,:);

    eulerZYX = [rz ry rx];

    rot_Welt_zu_TCP = inv(RotationUmZ(deg2rad(-eulerZYX(1,1))))*inv(RotationUmY(deg2rad(-eulerZYX(1,2))))*inv(RotationUmX(deg2rad(-eulerZYX(1,3)))); 
    ...
    phi_1 = CalculateAngleInXYPlane(Vorzugsrichtung,kelle_richtung_Welt_neu);

    phi_2 = 90 - (acos(dot([0 0 1],kelle_richtung_Welt_neu)/(norm(kelle_richtung_Welt_neu)))/pi * 180);
    ...
    phi1_min = min(simu_CFD_phi1_k9); %Um Welt in Sym zu drehen
