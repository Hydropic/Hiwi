...
problem = createOptimProblem('fmincon',...
    ...
    'nonlcon', @(optimization_values)constraintFcnValidation(optimization_values,init_ax_values), ...
    'lb',min_values,...
    'ub',max_values, ...
    'options',opts);
...
function [c,ceq] = constraintFcnValidation(optimization_values, input_AchsStellung)
    ...
    timeintervals = optimization_values(1:size(optimization_values,1)-1,1);
    base_points = optimization_values(:,2:size(optimization_values,2));
    input_AchsStellung = input_AchsStellung(:,2:size(optimization_values,2));
    ...
    [start_pos_backup, start_kelle_backup] = vorwaertskinematik(input_AchsStellung(1,:));
    [start_pos_new, start_kelle_new] = vorwaertskinematik(base_points(1,:));
    [end_pos_backup, end_kelle_backup] = vorwaertskinematik(input_AchsStellung(size(base_points,1),:));
    ...
    for i=1:size(base_points,2) %for every axis
         [t,veolocity,acceleration,jerk,time,place] = spline(base_points(:,i),timeintervals,false);
         if 0 ~= sum(place==0)
         ...
         ceq(end+1) = veolocity(place(1)) - 0;
         ceq(end+1) = veolocity(place(size(base_points,1))) - 0;
% % % % %            c(end+1) = abs(td(place(1)) - 0) - precision;
% % % % %            c(end+1) = abs(td(place(size(base_points,1))) - 0) -precision;   
    
         % KEEP START AND END- POINTS 
         ceq(end+1) = base_points(1,i) - input_AchsStellung(1,i); %TODO ggf als kellenposition
         ceq(end+1) = base_points(size(base_points,1),i) - input_AchsStellung(size(base_points,1),i);
% % % %            c(end+1) = abs(base_points(1,i) - input_AchsStellung(1,i)) - precision;
% % % %            c(end+1) = abs(base_points(size(base_points,1),i) - input_AchsStellung(size(base_points,1),i)) - precision;
    ...
    %      ceq(end+1) = sum(tddd<min_jerk(i))-0;  
         ...
         for j=1:size(base_points,1) %for every point on every axis
            ...
            %Formel prüfen
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %if Wert in Intervall = 0 sonst ungleich null mit grad.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            c(end+1) = veolocity(place(j)) - max_velocity(i);%velocity
            c(end+1) = min_velocity(i) - veolocity(place(j));
            c(end+1) = acceleration(place(j)) - max_acceleration(i);%acceleration
            c(end+1) = min_acceleration(i) - acceleration(place(j));   
            c(end+1) = jerk(place(j)) - max_jerk(i);%jerk
            c(end+1) = min_jerk(i) - jerk(place(j));   
         end
    end
