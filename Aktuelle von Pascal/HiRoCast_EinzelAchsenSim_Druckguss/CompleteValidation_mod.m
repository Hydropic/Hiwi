...
for i=1:length(t1)
    ...
    if start_z + variance_z > 180
        if rz < 0 
            rz = abs(180+rz)+180;
            %rz = 360 - rz; Altanative Variante
        end
    end 
    ...
    if ismember(i,place1)
       ...
       %beiden winkel überprüfen ob plausibel definiert
       ...
       if c(end) > 1
            fprintf('Bedingung krd phi1Max Verletzt ');
            fprintf(num2str(c(end)));
       end
       c(end+1) = phi1_min - phi_1;
       if c(end) > 1
            fprintf('Bedingung krd phi1min Verletzt');
            fprintf(num2str(c(end)));
       end
       c(end+1) = phi_2 - phi2_max;
       if c(end) > 1
            fprintf('Bedingung krd phi2max Verletzt');
            fprintf(num2str(c(end)));
       end
       c(end+1) = phi2_min - phi_2;
       if c(end) > 1
            fprintf('Bedingung krd phi2min Verletzt');
            fprintf(num2str(c(end)));
       end
    end
