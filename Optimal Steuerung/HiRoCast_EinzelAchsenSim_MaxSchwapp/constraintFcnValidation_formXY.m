function [c,ceq] = constraintFcnValidation_formXY(optimized_translational_values_5_6,init_optimization_values,optimized_translational_values, orientationsPendulum, passes, acceptableOrientationDifference)
global sumAngularDifference;

% plot(optimized_translational_values_5_6);
% plot(init_optimization_values);
% plot(optimized_translational_values);
% plot(orientationsPendulum);
% Update all valuse
[optimized_translational_values] = updateTimeAndAxes(optimized_translational_values_5_6, optimized_translational_values);
% optimized_translational_values(:,6) = -optimized_translational_values(:,6);

%% =========Feste Variablen==========================================
    % velocity GRAD/SEC
    max_velocity = deg2rad([300,400,400]);
    min_velocity = deg2rad([-300,-400,-400]);

    % acceleration GRAD/SEC2
    max_acceleration = deg2rad([150,200,200]);
    min_acceleration = deg2rad([-150,-200,-200]);
...
%% =========Kontrollen der einzelnen Achsgrenzen==============
    ...
    [position_TCP, angleTCP, optimization_values] = prepairValuse_5_6(optimized_translational_values, passes);
    ...
    % orientationsPendulum_XY_timeCorrected = orientationsPendulum;
    ...
    sumAngularDifference = sum(abs(angularDifference));
    save('sum_Angular_Difference.mat','sumAngularDifference');
