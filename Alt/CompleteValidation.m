% function [c,ceq] = completeValidationKurzzeitigAuskommentiert(timeintervals, base_points)
% print('Validation only base');
...
%     if i == size(base_points,1)  | i == size(base_points,1) -1
%         p = base_points(i,:);
%         v = base_points(i-1,:);
%         [pos, kelle, y_direction] = vorwaertskinematik(p);
%         [posv, kellev, y_directionn] = vorwaertskinematik(v);
%         [acceleration, globaleDirection, direction] = Beschleunigung(posv, pos, pos, (time1(place1(i))-time1(place1(i-1))),p);
%     elseif i == 1
%         p = base_points(i,:);
%         n = base_points(i+1,:);
%         [pos, kelle, y_direction] = vorwaertskinematik(p); 
%         [posn, kellen, y_directionn] = vorwaertskinematik(n); 
%         [acceleration, globaleDirection, direction] = Beschleunigung(pos, pos, posn, (time1(place1(i+1))-time1(place1(i))),p);     
%     else
%         p = base_points(i,:);
%         n = base_points(i+1,:);
%         nn = base_points(i+2,:);
...
%         [acceleration, globaleDirection, direction] = Beschleunigung(pos, posn, posnn, (time1(place1(i+1))-time1(place1(i))),p);
%     end
% 
...
% for i=1:size(base_points,1) 
%     %c(end+1) = abs(abs(Acceleration(i)) - abs(predictedAcceleration(i)/1000)) - variance_a;
%     c(end+1) = abs(abs(Acceleration(i)) - abs(predictedAcceleration(i)/1000)) - variance_a;
%     fprintf('Ist: %f Soll: %f\n',abs(Acceleration(i)),abs(predictedAcceleration(i)/1000))
% end
% 
% % Sum-Constraint: amount of violations should be =0
% %ceq(end+1) = sum(abs(abs(predictedAcceleration/1000)-abs(Acceleration))>variance_a) - 0;
% fprintf('Validation correct: %d / %d \n',sum(abs(abs(predictedAcceleration/1000)-abs(Acceleration))<=variance_a), size(base_points,1) );
% end
% 


function [c,ceq] = completeValidation(timeintervals, base_points)
print('Validation spline + base');
%Validates of a path sloshes
%   Input: Basepoints, Computes spline, iterates over all points (extra
%   cases first and last point), sets constraints on forward kinematics
%   Outputs: Constraint Arrays: sum of validation and special values at
...
for i=1:length(t1)
    if i == length(t1) | i == length(t1)-1
        ...
        [pos, kelle, y_direction] = vorwaertskinematik(p);
        [posv, kellev, y_directionn] = vorwaertskinematik(v);
        [acceleration, globaleDirection, direction] = Beschleunigung(posv, pos, pos, (time1(i)-time1(i-1)),p);
    elseif i == 1
        p = [t1(i), t2(i), t3(i), t4(i), t5(i), t6(i)];
        n = [t1(i+1), t2(i+1), t3(i+1), t4(i+1), t5(i+1), t6(i+1)];
        [pos, kelle, y_direction] = vorwaertskinematik(p); 
        [posn, kellen, y_directionn] = vorwaertskinematik(n); 
        [acceleration, globaleDirection, direction] = Beschleunigung(pos, pos, posn, (time1(i+1)-time1(i)),p);     
    else
        p = [t1(i), t2(i), t3(i), t4(i), t5(i), t6(i)];
        n = [t1(i+1), t2(i+1), t3(i+1), t4(i+1), t5(i+1), t6(i+1)];
        ...
        [posn, kellen, y_directionn] = vorwaertskinematik(n); 
        [posnn, kellev, y_directionv] = vorwaertskinematik(nn); 
        [acceleration, globaleDirection, direction] = Beschleunigung(pos, posn, posnn, (time1(i+1)-time1(i)),p);
    end
    ...
    if ismember(i,place1)
        ...
        c(end+1)= ry - max(simulation_data.rotationY); %Y
        c(end+1)= min(simulation_data.rotationY) - ry;
        % Will be deleted: c(end+1) = abs(start_z - rz) - variance_z; %Z
        c(end+1)= acceleration - (max(simulation_data.acceleration)/1000); %A
        c(end+1)= (min(simulation_data.acceleration)/1000) - acceleration;
        %TODO Polarkoordinaten/Vorzugsrichtungsvarianz
    end
...
% Setting constraints for the basepoints
for i=1:size(base_points,1) 
...
% Sum-Constraint: amount of violations should be =0
% Will be deleted: ceq(end+1) = sum(abs(abs(predictedAcceleration/1000)-abs(Acceleration))>variance_a) - 0;
fprintf('Validation: %d / %d \n',sum(abs(abs(predictedAcceleration/1000)-abs(Acceleration))>variance_a), length(t1));
end
